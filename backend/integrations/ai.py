...
class LlmApiClient:
    ...
    def get_habits_generation_prompt(
        ...
        prompt = f"""Ты — эксперт по продуктивности и формированию привычек.
            ...
            быть конкретным действием (макс. 100 символов).
            ...
            НЕ добавляй никаких вступлений, пояснений, извинений или markdown-оберток (таких как ```json).
            Только чистый JSON.
    ...
    def get_ai_report_prompt(
        ...
        prompt = f"""Ты — профессиональный AI-коуч по продуктивности и достижению целей.
            Твоя задача: проанализировать данные о прогрессе пользователя за конкретную неделю и
            составить мотивирующий, структурированный еженедельный отчет.
            ...
            2. 🏆 **Главные победы**: Конкретные привычки или цели, где пользователь
            показал отличный результат или регулярность.
            3. ⚠️ **Зоны роста**: Привычки, которые проседали, были пропущены или остались без внимания.
            Будь конструктивен, не ругай, а предлагай решения.
            4. 💡 **Фокус на следующую неделю**: Один конкретный, выполнимый совет, основанный на данных
            (например, "Попробуй снизить целевое время для X, чтобы войти в ритм", или "Ты отлично справляешься с Y,
            попробуй добавить Z").
            ...
            - НЕ добавляй никаких вступлений вроде "Вот твой отчет:" или заключений.
              Начинай ответ сразу с раздела "📊 **Общая сводка**".
        """
        return prompt
